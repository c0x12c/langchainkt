...
class E5SmallV2QuantizedEmbeddingModelTest {
  ...
  fun should_embed() {
    val model: EmbeddingModel = E5SmallV2QuantizedEmbeddingModel()
  ...
  @Test
  @Disabled("Temporary disabling. This test should run only when this or used (e.g. langchain4j-embeddings) module(s) change")
  fun embedding_should_have_similar_values_to_embedding_produced_by_transformers_python_lib() {
    val model: EmbeddingModel = E5SmallV2QuantizedEmbeddingModel()
    val embedding = model.embed("query: I love transformers.").content()
    assertThat(embedding.vector()[0]).isCloseTo(-0.0663562790f, org.assertj.core.data.Percentage.withPercentage(16.0))
    assertThat(embedding.vector()[1]).isCloseTo(0.0153982891f, org.assertj.core.data.Percentage.withPercentage(16.0))
    assertThat(embedding.vector()[382]).isCloseTo(-0.0412562378f, org.assertj.core.data.Percentage.withPercentage(16.0))
    assertThat(embedding.vector()[383]).isCloseTo(-0.0130311009f, org.assertj.core.data.Percentage.withPercentage(16.0))
  }
  ...
  fun should_produce_normalized_vectors() {
    val model: EmbeddingModel = E5SmallV2QuantizedEmbeddingModel()
    val oneToken = "hello "
    assertThat(magnitudeOf(model.embed(oneToken).content())).isCloseTo(1.0f, org.assertj.core.data.Percentage.withPercentage(0.01))
    assertThat(magnitudeOf(model.embed(oneToken.repeat(999)).content())).isCloseTo(1.0f, org.assertj.core.data.Percentage.withPercentage(0.01))
  }
}
